...
- Releases are **automated** by `.github/workflows/release.yml` — do NOT hand-cut releases (`gh release create`) or force-move existing tags; that fights the workflow and produces asset-less duplicates. Every CI-passing merge to `main` auto-cuts the next **patch** (`latest v-tag + 1`): the workflow builds the binaries + `checksums`, syncs the `plugin.json` versions, and publishes the GitHub Release as `github-actions[bot]`. To bump **minor/major** (e.g. `v0.6.x → v0.7.0`), push only the tag (`git tag vX.Y.0 && git push origin vX.Y.0`) — the same workflow builds that release; the bot then auto-patches (`vX.Y.1`, …) from there on subsequent merges. Skill/CLI/deploy/plugin edits are user-facing and ride the same auto-release; downstream picks them up via `claude plugin update workbuddy` and `workbuddy deploy upgrade`. The combined K8s image (`deploy/docker/Dockerfile`) is published separately to `docker.io/opspai/workbuddy-agentm` and is NOT built by `release.yml`.
