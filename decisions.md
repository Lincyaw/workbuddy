...
- `[L2]` After verifying the app-server path end-to-end, the legacy `codex exec` launcher and `WORKBUDDY_CODEX_BACKEND` compatibility switch were removed entirely, so `runtime: codex` now resolves directly to the JSON-RPC backend and CLI/session metadata no longer report the stale `codex-exec` runtime name.
