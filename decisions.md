...
- `[L3]` Codex `app-server` is now multiplexed: a single worker-wide `codex app-server --listen stdio://` child hosts one JSON-RPC thread per agent session instead of one process per session. This supersedes the 2026-04-20 `[L4][flagged]` decision — schema re-inspection showed `ThreadStartParams` accepts per-thread `cwd`/`model`/`sandbox`/`approvalPolicy`/`config` and `CommandExecParams` accepts per-request `env`, so the per-process isolation premise no longer holds. The shared process is owned by `internal/agent/codex.Backend`, started lazily on first `NewSession`, torn down from `serve`/`worker` shutdown via `runtime.Registry.Shutdown` → `AgentBridgeRuntime.Shutdown` → `Backend.Shutdown`; individual `session.Close` only calls `turn/interrupt` + `thread/archive` and never signals the child. This cuts per-session startup cost (~1s + 150MB RSS per concurrent session) at worker `--concurrency > 1`, while cancellation semantics and per-session event capture remain unchanged.
