...
| `review-agent` | review | claude-code | ✅（内置 `go build/vet/test` 作为 merge gate） |
| `codex-dev-agent` | dev | codex | ✅（codex-exec Event v1 已接通；用于草稿 PR 开发流） |
| `codex-review-agent` | review | codex | ✅（同 review-agent，codex runtime） |

> 注：早期有独立的 `test-agent` / `codex-test-agent`，现已合并到 review 阶段——reviewer 自己跑测试，不再需要独立 testing 状态。
...
- `routing-dev-to-review.md`
- `routing-review-to-done.md`
- `pr-creation-guidelines.md`
- `commit-message-style.md`
- `gh-cli-cheatsheet.md`

agent md 通过 `prompt_includes: [routing-dev-to-review, pr-creation-guidelines]` 引用。具体加载机制留 vNext 实现时定。
