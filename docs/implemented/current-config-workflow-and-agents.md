...
- `status:done` 是 post-merge 终态 label；关闭 issue 由确认 merge 的 review-agent 或执行 merge 的人负责，状态机不会代 agent 自动关闭 issue

当前 loader 对 workflow state 中的未知 key 采用“静默忽略”策略；它会继续解析
workflow，但不会给这些额外字段任何运行时语义。因此像旧版 `action: close_issue`
这样的 decorative config 不应继续出现在样例 workflow 中。
