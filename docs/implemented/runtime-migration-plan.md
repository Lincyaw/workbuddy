...
- 长驻 runtime / app-server 连接池
- Per-worker / per-repo app-server 连接池
- 动态审批回调
- 实时 UI 与事件回放
...
- `internal/launcher/agent_bridge.go` — launcher 到 agent.Backend 的桥接
- `internal/agent/codex/backend.go` — Codex app-server runtime
