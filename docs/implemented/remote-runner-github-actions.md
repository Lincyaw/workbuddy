...
- `events-v1.jsonl`
  ...
  - 必须存在，除非以 `codex-exec.jsonl` 作为 fallback
- `codex-exec.jsonl`
  - 当没有 `events-v1.jsonl` 时作为 fallback session artifact
- `workbuddy-result.json`
  - 可选；用于回传 `exit_code`、`last_message`、`meta`、`session_ref`、`token_usage`

如果 artifact 中既没有 `events-v1.jsonl`，也没有 `codex-exec.jsonl`，remote runner 会直接返回错误，而不是把纯文本日志伪装成 session artifact。
