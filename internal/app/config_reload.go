...
// NotifierRuntime wraps the notifier.Service with live-reload semantics: on
// Apply, a new notifier is spun up with a fresh cancellable context and the
// previous one is cancelled so alert delivery never duplicates or drops
// mid-reload.
...
// NewNotifierRuntime builds a NotifierRuntime, applies the initial
// notifications config, and starts the notifier goroutine.
...
// ValidateNotificationsConfig checks that a NotificationsConfig can be
// turned into a notifier without mutating any existing state.
...
// Apply swaps in a new notifier configuration atomically.
...
// ConfigReloadSummary is the JSON shape returned by the /api/v1/config/reload
// endpoint and emitted into the event log on every reload attempt.
...
// CoordinatorConfigRuntime owns the coordinator's live config: the current
// FullConfig, the watcher that triggers reloads on disk changes, and the
...
// Current returns a deep copy of the currently-applied FullConfig.
...
// Reload re-reads configDir from disk and applies it atomically. On any
// error the previous configuration is restored.
...
// StartCoordinatorConfigWatcher wires an fsnotify watcher that triggers a
// debounced reload of the given runtime when files in configDir change.
